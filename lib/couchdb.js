...
CouchDB.prototype.autoupdate = function(models, cb) {
  ...
  function autoUpdateOneModel(model, cb) {
    ...
    async.series([
      function prepPropertyLevelIndex(cb) {
        createPropertyLevelIndex(propertyLevelIndexes, cb);
      },
      function prepModelLevelIndexes(cb) {
        createModelLevelIndexes(modelLevelIndexes, cb);
      },
    ], cb);

    function createPropertyLevelIndex(indexes, cb) {
      async.eachOf(indexes, function(value, index, cb) {
        var fields = [index.split('_index')[0]];
        createIndex(index, fields, cb);
      }, function(err) {
        cb(err);
      });
    }

    function createModelLevelIndexes(indexes, cb) {
      async.eachOf(indexes, function(value, index, cb) {
        var keys = value.keys;
        var fields = [];
 ...
 */
CouchdbDB.prototype.getModelIndexes = function(model, cb) {
  var self = this;
  var mo = self.selectModel(model);
